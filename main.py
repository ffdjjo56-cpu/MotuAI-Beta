...
# Автоматически собираем ВСЕ переменные, которые начинаются на GEMINI_KEY_
...
async def handle(request):
    return web.Response(text=f"Lal is chaotic. Active keys: {len(ALL_KEYS)}")
...
# --- Хаотичный запрос к Gemini 3 Flash Preview ---
async def get_gemini_response(text):
    # Делаем копию списка, чтобы перемешивать её для каждой попытки
    ...
    for i, api_key in enumerate(keys_pool):
        try:
            ...
            logger.warning(f"Один из {len(ALL_KEYS)} ключей устал. Пробую следующий...")
            continue
        except Exception as e:
            logger.error(f"Ошибка на случайном ключе: {e}")
            continue

    return "слыш у меня даже ключи все сдохли от твоей рожи так что иди гуляй"
...
async def main():
    await start_server()
    logger.info(f"Lal запущен. Загружено ключей: {len(ALL_KEYS)}")
